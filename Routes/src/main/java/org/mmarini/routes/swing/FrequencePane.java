...
public class FrequencePane extends Box {
	...
	private BoundedRangeModel frequenceModel;
	...
	public FrequencePane() {
		...
		frequenceModel = new DefaultBoundedRangeModel(0, 0, 0,
				FREQUENCES.length - 1);
		createContent();
	}
	...
	private void createContent() {
		...
		box.add(new JLabel(Messages
				.getString("FrequencePane.frequenceLabel.text"))); //$NON-NLS-1$
		box.add(createHorizontalGlue());
		JSlider freqSlider = new JSlider(SwingConstants.HORIZONTAL);
	...
	public void setFrequence(double frequence) {
		...
		for (int i = 0; i < FREQUENCES.length; ++i) {
			double e = Math.abs(frequence - FREQUENCES[i]);
			if (e < error) {
				idx = i;
				error = e;
			}
