...
public class Painter {
	...
	private Shape veicleShape;

	private Shape siteShape;

	private Shape edgeEndPoint;
	...
	/**
         ...
         */
	public Painter() {
		stroke = new BasicStroke((float) EDGE_WIDTH, BasicStroke.CAP_ROUND,
				BasicStroke.JOIN_ROUND);
		thinStroke = new BasicStroke(0f, BasicStroke.CAP_ROUND,
				BasicStroke.JOIN_ROUND);
		line = new Line2D.Double();
		veicleShape = new Rectangle2D.Double(-VEICLE_LENGTH * 0.5,
				-VEICLE_WIDTH * 0.5, VEICLE_LENGTH, VEICLE_WIDTH);
		siteShape = new Ellipse2D.Double(-NODE_SIZE * 0.5, -NODE_SIZE * 0.5,
				NODE_SIZE, NODE_SIZE);
		edgeEndPoint = new Ellipse2D.Double(-EDGE_WIDTH * 0.5,
				-EDGE_WIDTH * 0.5, EDGE_WIDTH, EDGE_WIDTH);
	}
	 ...
	 * @param edge
	 */
	public void paintCursorEdge(MapEdge edge) {
		Point2D beginLocation = edge.getBeginLocation();
		Point2D endLocation = edge.getEndLocation();
	 ...
	 * @param edge
	 */
	public void paintCursorEdgeEnds(MapEdge edge) {
		Point2D beginLocation = edge.getBeginLocation();
		Point2D endLocation = edge.getEndLocation();
	 ...
	 * @param edge
	 */
	public void paintEdge(MapEdge edge) {
	 ...
	 * @param color
	 */
	public void paintEdge(MapEdge edge, Color color) {
	 ...
	 * @param color
	 */
	public void paintEdge(Point2D from, Point2D to, Color color) {
	 ...
	 * @param size
	 */
	public void paintGrid(Rectangle2D bound, double size) {
		double x0 = bound.getMinX();
		double x1 = bound.getMaxX();
		double y1 = bound.getMaxY();
		double y0 = bound.getMinY();
		Color minorColor = reversed ? MINOR_GRID_REVERSED_COLOR
				: MINOR_GRID_COLOR;
		Color majorColor = reversed ? MAJOR_GRID_REVERSED_COLOR
				: MAJOR_GRID_COLOR;
		graphics.setColor(minorColor);
		graphics.setStroke(thinStroke);
		for (double x = Math.floor(x0 / size) * size; x <= x1; x += size) {
			double xg = Math.floor(x / size / 10.) * 10. * size;
			if (x == xg) {
		...
		for (double y = Math.floor(y0 / size) * size; y <= y1; y += size) {
			double xg = Math.floor(y / size / 10.) * 10. * size;
			if (y == xg) {
				graphics.setColor(majorColor);
			} else {
				graphics.setColor(minorColor);
	 ...
	 * @param vecy
	 */
	public void paintModule(Module module, Point2D location, double vecx,
			double vecy) {
	 ...
	 * @param center
	 */
	public void paintNodeCursor(Point2D center) {
	 ...
	 * @param color
	 */
	private void paintShape(Shape shape, Point2D location, Color color) {
		graphics.setColor(color);
		AffineTransform tr = graphics.getTransform();
	 ...
	 * @param color
	 */
	public void paintSite(Point2D location, Color color) {
	 ...
	 * @param center
	 */
	public void paintSiteCursor(Point2D center) {
	 ...
	 * @param color
	 */
	public void paintVeicle(Point2D location, Point2D vec, Color color) {
		graphics.setColor(color);
		AffineTransform old = graphics.getTransform();
		AffineTransform tr = graphics.getTransform();
