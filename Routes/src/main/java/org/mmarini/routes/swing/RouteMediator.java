...
public class RouteMediator {
    ...
    */
	...
	public void dump() {
		try {
			...
			JOptionPane.showMessageDialog(mainFrame,
					Messages.getString("RouteMediator.dumpMessage.text")); //$NON-NLS-1$
		} catch (Exception e) {
			e.printStackTrace();
			showError(e);
		}
		...
		if (choice == JFileChooser.APPROVE_OPTION) {
			...
			if (!file.canRead()) {
				...
				try {
					...
					e.printStackTrace();
					showError(
							Messages.getString("RouteMediator.parseError.message"), new Object[] { e.getMessage(), //$NON-NLS-1$
	...
	public String retrieveNodeName(MapNode node) {
		...
		return MessageFormat.format(defaultNodeName,
	...
	public void showTrafficInfos() {
		...
		JOptionPane
				.showMessageDialog(
						mainFrame,
						pane,
						Messages.getString("RouteMediator.trafficInfoPane.title"), JOptionPane.INFORMATION_MESSAGE); //$NON-NLS-1$	
		startSimulation();
	}
