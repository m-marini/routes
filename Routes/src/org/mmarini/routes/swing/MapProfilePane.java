...
public class MapProfilePane extends Box {
	...
	private SpinnerNumberModel siteCount;
	private DefaultComboBoxModel mapSizeModel;
	private DefaultComboBoxModel difficultyModel;
	...
	public MapProfilePane() {
		...
		mapSizeModel = new DefaultComboBoxModel(
				new Object[] {
						Messages.getString("MapProfilePane.mapSize.small.text"), Messages.getString("MapProfilePane.mapSize.medium.text"), Messages.getString("MapProfilePane.mapSize.large.text") }); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		difficultyModel = new DefaultComboBoxModel(
				new Object[] {
						Messages
								.getString("MapProfilePane.difficulty.easy.text"), //$NON-NLS-1$
						Messages
								.getString("MapProfilePane.difficulty.medium.text"), Messages.getString("MapProfilePane.difficulty.hard.text") }); //$NON-NLS-1$ //$NON-NLS-2$
		siteCount = new SpinnerNumberModel(4, 2, 20, 1);
		mapSize = new double[] { SMALL_SIZE, NORMAL_SIZE, LARGE_SIZE };
		minWeight = new double[] { EASY_MIN_WEIGHT, NORMAL_MIN_WEIGHT,
				DIFFICULT_MIN_WEIGHT };
		frequence = new double[] {
				EASY_FREQUENCE / ((1d + EASY_MIN_WEIGHT) / 2d),
	...
	private void createContent() {
		...
		box
				.add(new JLabel(Messages
						.getString("MapProfilePane.sizeLabel.text"))); //$NON-NLS-1$
		...
		box.add(new JLabel(Messages
				.getString("MapProfilePane.siteCountLabel.text"))); //$NON-NLS-1$
		box.add(createHorizontalGlue());
		...
		box = createHorizontalBox();
		box.add(new JLabel(Messages
				.getString("MapProfilePane.difficultyLabel.text"))); //$NON-NLS-1$
		box.add(createHorizontalGlue());
	 ...
	 * @param profile
	 */
	public void retrieveProfile(MapProfile profile) {
		profile.setSiteCount(siteCount.getNumber().intValue());
		int sizeIdx = mapSizeModel.getIndexOf(mapSizeModel.getSelectedItem());
		int diffIdx = difficultyModel.getIndexOf(difficultyModel
				.getSelectedItem());
	 ...
	 * @param difficultOnly
	 */
	public void setDifficultyOnly(boolean difficultOnly) {
