...
public class SwingUtils {
	 ...
	 *            the control value
	 * @param saturation
	 *            the saturation of result color
	 * @return the iride color
	 */
	public Color computeColor(double value, double saturation) {
		double b = interpolate(value, BRIGHTNESS_ZERO, BRIGHTNESS_ONE);
		double h = interpolate(value, HUE_ZERO, HUE_ONE);
		Color color = Color.getHSBColor((float) h, (float) saturation,
				(float) b);
		return color;
	}
	 ...
	 * 
	 * @param action
	 *            the action to be initialized
	 * @param key
	 *            the key identifier in the message file
	 */
	public void initAction(Action action, String key) {
		...
		if (!msg.startsWith("!")) { //$NON-NLS-1$
			action
					.putValue(Action.MNEMONIC_KEY, Integer.valueOf(msg
							.charAt(0)));
		}
		msg = Messages.getString(key + ".smallIcon"); //$NON-NLS-1$ 
		if (!msg.startsWith("!")) { //$NON-NLS-1$
			URL url = getClass().getResource(msg);
			if (url != null) {
	 ...
	 *            the control value
	 * @param zeroLevel
	 *            the start value (associated to the 0 level of control value)
	 * @param oneLevel
	 *            the end value (associated to the 1 level of control value)
	 * @return the linear interpolated value
	 */
	private double interpolate(double control, double zeroLevel, double oneLevel) {
